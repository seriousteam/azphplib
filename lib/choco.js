...
X.zones = {
	...
	, makeZone: function(e, name, parent) {
		...
		if(name === '+') return this.lastZone = zone;
		var z = {
			items: []
			, childs: []
		...
		z.index[name] = z;
		zone.childs.push(z)
		return this.lastZone = e.dynZone = z;
	}
	...
	, makeVal: function(e, name, func, agg) {
		if (name && name[0] === '+')
			return this.makeZone(e, name, agg);
		...
		if(func) {
			...
			func = func.replace(/@V\(\)/g, 'this.source.V()')
