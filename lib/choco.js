...
Element.prototype.Vn = function() { return +this.V().replace(/[^\d.-]/g,''); }
Element.prototype.Vi = function() { return parseInt(this.V().replace(/[\d.-]/g,'')||'0'); }
...
@*/
		return f.apply(self, args).then(
			function(r) { state.fails = 0; return r; }
		,
			function(x) {
				var m = mode_func.apply(null, args)
				++state.fails;
				if(m === undefined) throw x;
				m(state);
				return ret.apply(self, args)
			}
		)
	}
	return ret;
...
X.XHR_retry_mode = function(url, content) {
			return;
			// undefined - no retry at all
			return function(fails) {
					//if(state.fails < silent) return;
					alert('Communication error!'); 
					// function can:
					//   -- reload page
					//   -- reset tries count (state.fails)
					// ... when function returns XHR go to next try
			}
	}
...
X.zones = {
	...
	, makeVal: function(e, name, func, agg, prevVar) {
		if (name && name[0] === '^')
		...
		if(func) {
			var strings = [];
			func = this.prepareStrings(func, strings);
			func = func.replace(/\[\.\]/g, '[' + prevVar + ']')
			//console.log(func, prevVar);
			...
			func = func.replace(/(^|\s)dyn_regexp\s*\(/g,' PHPregexp( ');
			func = func.replace(/(^|\s)(min|max)\s*\(/g,' Math.$1( ');
			func = this.stringsBack(func, strings);
			func = new Function("return " + func.trim());
		}
		var bind = '';
		var v = {
			name: name
			...
			, target: name ?
				(
					{ 
						...
						, '@save' :
							function(v) { X.zones.saveCtrl(e, v); return v; }
					}[name]
					|| name.match(/^@attr-(.*)/) && (bind = RegExp.$1) && 
						function(v) { e.setA(bind,v); return v; }
					|| name.match(/^@attrEval-(.*)/) && (bind = RegExp.$1) && 
						function(v) { e.evalHere('@'+bind,v); return v; }
					|| name.match(/^@eval-(.*)/) && (bind = RegExp.$1) && 
						function(value) { var target = e; eval(bind); return value; }
					|| nop
				) 
				: function(v) { e.setV(v) }
			, zone: zone
		...
		for(var i = 0; i < a.length; ++i) {
				...
				for(var j = 0; j < s.length; ++j) {
					...
					if(m = s[j].match(/^\s*([^:=]*)(\s*:\s*([^:=]+))?(\s*=([\s\S]*))?/)){
						...
						var agg = m[3];
						var func = m[5];
						this.makeVal(a[i], name, func , agg, prevVar);
						prevVar = name;
				   } 
		  }
	  }
	...
	, prepareStrings : function (x, arr) {
		return x.replace(/'[^']*(''[^']*)*?'/g, function(a) {
			arr.push(a); return "'"+(arr.length-1)+"'";
			});
	}

	, stringsBack: function (s, arr) { return s.replace(/'(\d+)'/g, function(a, n) { return arr[+n]; }); }

	, saveCtrl: function(target, value) { 
		var prevValue = target.V();
		if( prevValue !== '' && !X.isString(value) ) prevValue = +prevValue;
		//console.log(target, value, prevValue);
		if( prevValue !== value) {
			//console.log('saved');
			target.setV(value)
			saveControlVal(target);
		}
	}
}
