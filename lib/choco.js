...
Element.prototype.setA = function(name, v) { 
	...
	else this.setAttribute(name || this.lastAttr, v); 
	return this; 
}
...
Element.prototype.setDN = function(st) {
	if(st === toggle)
		...
		if(st && m.A('ref')) {
			...
			e.setA('loading','');
			reloadElement(m).done(function(txt) { e.removeAttribute('loading'); e.setDN(true); });
		} else {
...
Element.prototype.setDN_TR = function(st) {
	if(st === toggle)
		...
		if(st && m.A('ref')) {
			...
			if(!m.A('ref-src')) m.setA('ref-src', m.evalHere(m.V()));
			e.setA('loading','');
			reloadElement(m).done(function(txt) { e.removeAttribute('loading'); e.setDN_TR(true); });
		} else {
...
Element.prototype.showXModal = function() {
	...
	for(var e = this;  e /*&& e.offsetParent*/; e = e.offsetParent) {
		var st = e.currentStyle?e.currentStyle : getComputedStyle(e,"display");
		if(st['display'] == 'table-cell' || st['display'] == 'table-row') continue;

		var cover = document.createElement("DIV");
		cover.className = "coverBox";
		if(!e.offsetParent || !e.offsetParent.offsetParent)
			cover.style.position = 'fixed';
		if(e.offsetParent)
			e.insertBeforeIt(cover);
		else
			e.insertBefore(cover, e.firstChild);
...
Element.prototype.reloadIt = function (once) {
	...
	if(!m.A('ref')) return X.defer(m); //incompatible with previous version, but scripts & editing did check
	if(once===undefined) once = m.A('ref')!='EP' && m.A('ref')!='P';
	var u = m.A('ref')=='E' || m.A('ref')=='EP' ? m.A('ref-src').evalParts(m) : m.A('ref-src');
	return X.GET(u)
		.done(function(txt) {
			if(once) m.setA('ref', '');
...
X.zones = {
	...
	, zoneOf: function(e) {
		e = e.UA('dyn-zone');
		return e && e.dynZone || this.root;
	}
	, makeZone: function(e) {
		var zone = this.zoneOf(e.parentNode);
		var z = {
			items: []
			, childs: []
	...
	, makeVal: function(e, name, func, agg, bind) {
		...
		var v = {
			...
			, target: bind && e
			, zone: zone
			, aggregateTo: agg && zone.parent && zone.parent.index[agg]
			, age: this.age
		}
		zone.items.push(v);
		zone.index[name] = v;
		if(func) {
			func = func.replace(/\$([a-zA-Z_][a-zA-Z0-9_]*)/g,'this.zone.index["$1"].value');
			func = func.replace(/@V\(\)/g, 'this.target.V()')
			func = func.replace(/@N\(\)/g, 'this.target.Vn()')
	...
	, setVal: function(v, value, make) {
		if(v.value !== value) {
			...
			v.value = value;
			if(v.target)
				v.target.setV(value === undefined ? '' : value); //or setter
			if(!make)
				this.refresh(v.zone);
			if(v.aggregateTo
	...
	, recalc: function(v) {
		if(v.dynVal) return this.recalc(v.dynVal);
		if(v.func) {
			try {
				var val = v.func();
			} catch(e) {
	...
	, setup: function(e) {
		this.age ++;
		var a1 = [ [e] , e.QSA('[dyn-zone],[dyn-val]') ];
		while(a = a1.shift())
		for(var i = 0; i < a.length; ++i) {
			if(a[i].hasA('dyn-zone')) 
				this.makeZone(a[i]);
			if(a[i].A('dyn-val')) {
				var s =  a[i].A('dyn-val').split(';')
				for(var j = 0; j < s.length; ++j) {
					var m;
					if(m = s[j].match(/^\s*([a-zA-Z_][a-zA-Z_0-9]*)(\s*:\s*([a-zA-Z_][a-zA-Z_0-9]*))?(\s*=(.*))?/)){
						var name = m[1];
						var agg = m[3];
						var func = m[5];
						this.makeVal(a[i], name, func , agg, j === 0);
					}
				}
			} 
		}
