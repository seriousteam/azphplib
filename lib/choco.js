...
Date.smartParse = function (s, as_struct_or_str) { //true - struct, false - str, undefined - date
	if(!s) return as_struct_or_str === false ? s : null;
	...
	return as_struct_or_str? parsed :
		as_struct_or_str === false ? 
			parsed.y + '-' + parsed.m.pad(2) + '-' + parsed.d.pad(2) + 
			(parsed.h ? ' ' + parsed.h.pad(2) + ':' + parsed.m.pad(2)
				+ (parsed.s ? ':' + parsed.s.pad(2) : '')
			: ''
			)
...
X.zones = {
	...
	, makeVal: function(e, name, func, agg) {
		...
		if(func) {
			func = func.replace(/\[:\]/g, 'this.source.V()')
		...
		var v = {
			...
			, target: name ?
				(
					name === '@check' ?
						function(v) { return e.dyn_check = v; }
						: nop
				) 
				: function(v) { e.setV(v) }
			, zone: zone
