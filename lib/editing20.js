...
function findRid(e) {
	...
	function findFirstSample(cont) {
		var e = cont.firstElementChild;
		while(e && !e.hasA("sample"))
			e = e.nextElementSibling;
		if(e) return e;
		for(var e = cont.firstElementChild; e; e = e.nextElementSibling) {
			var r = findFirstSample(e);
			if(r) return r;
		}
		return null;
	}
	...
	function findPreviousSample(e) {
		...
		do {
			...
			while(e = e.previousElementSibling)
				if(s = findFirstSample(e)) return s;
	...
	return function(ptxt) { var txt = ptxt || '';
		...
		if(m = txt.match(/^E: ([\s\S]*)/))
			...
			if(list_item) { //from makeChoose
				...
				if(g.QS("A"))
					saveControlVal( g.QS("A"), list_item, true);
				g.setA("cmd", m[2]);
...
function checkCtrl(e) {
	...
	if(e.tagName == "A" && e.hasA("rid") && ckdefs.required && 
...
function getCtrlValue(e) {
	...
	if(e.hasA("save_val")) {
		...
		pval = e.value;
	} else if(e.tagName == "INPUT" && (e.A("vtype")=='2' || e.A("vtype")=='3' || e.A("vtype")=='3M' 
			|| e.A("vtype")=='two' || e.A("vtype")=='three' || e.A("vtype")=='threeM')) {		
...
function setWithMenu(btn) {
	...
	var m =  choose? btn.nextElementSibling : btn;
	if(m.A('ref')) {
	...
	var lastSelect = choose ? m.previousElementSibling : btn;
	var flt = m.A("filter");
	m.setD(false);
	if(m.tagName == 'MENU') {
...
function prepareControls(cnt, init) {
	...
	for(var i = 0; i < a.length; ++i) {
		...
		ne.innerHTML = 
			...
			+' name=lobload_'+i+' ></iframe>'
			+'<div close-box=Y display=N modal=Y>'
			+'<form method="POST" enctype="multipart/form-data" target=lobload_'+i+' >'
	...
	for(var i = 0; i < a.length; ++i)
	{
		...
		if(e.tagName == "INPUT" || e.tagName == "TEXTAREA" || e.tagName == "SELECT") {
			if(e.tagName == "INPUT" && e.A("vtype")=='D') {
				if( !(e.A("add_button")=='N') && window.d3)//add_button=table
				{
					// set mask
					/*
					//need DMY
...
function makeReady(e, fld, dt, ext) {
...
function onchooseFileForm(e) {
	...
	for(var i = 0; i< ft.length; ++i)
		ft[i] = '\\.'+ft[i];
	if(e.value.match(new RegExp('('+ft.join('|')+')$', 'i'))) 
		e.tagUpward('FORM').submit(); 
		//alert(e.value);
	else alert('Не тот тип файла: нужен '+c.A('filetypes'))
}
...
function applyFuncFilter(e, offset, place, direct_txt) {
	...
	var a = e.QSA('[filter_ctrl]');
	for(var i =0; i < a.length; ++i)
		if(a[i].A('filter_ctrl'))
			mode[ a[i].A('filter_ctrl') ] = 
				(
				a[i].type==='checkbox' ? a[i].checked && a[i].value :
				a[i].hasA('rid') ? a[i].A('rid') :
				a[i].value && a[i].value.trim() 
				)|| undefined;
		else {
			var processedValue = { value: a[i].value }
			var aa = a[i].attributes;
			var ab = [];
			var ac = [];
	...
	if(cond) {
		if(e.QS('[filter_ctrl-replace]'))
			if(uri.URLParam('cmd'))
				cond.args.unshift('*WHERE '+cond.where);
			else
				uri = uri.setURLParam('cmd', '*WHERE '+cond.where);
		else
			if(uri.URLParam('cmd'))
				cond.args.unshift('*WHERE '+cond.where);
			else
...
function chooseWithFuncFilter(e, place) {
	var fdef = e.A('filter_def');
	if(e.prevRequest) e.prevRequest.object.abort();
	var mode = {}
		
	var v = e.searchbox.value;
	var processedValue = { value: v }
	...
	return e.prevRequest = X.XHRfeedback(place||e,'GET', uri);
}
...
var Calendar = (function() {
	...
	function calendarElement(cont, input, D, onOpen, onClose) {
		var self = this;
		self.D = D;
		self.IN = input;
		self.CAL = d2.S(cont)
		...
		{
			date = date || (empty(self.D) ? ymd( new Date() ) : self.D );
			var calend = self.CAL;
			...
			.AP("span")
				...
				.A("today", function(d) { if(zoom==="day") { if(eq(d, new Date() )) { return 1 } } })
			...
			if(todaycontrol.E() && zoom==="day")
				todaycontrol = calend.S( gte( todate(date), new Date() ) ? "[afterfirstelem]" : "[prelastelem]")
			
			todaycontrol.AP("span")
				.A("class","todaycontrol")
				.CK(function(e) {
					self.makeChoose(new Date(), "day");
				...
				.AP("span")
					.H(function(d) {
						return {
							...
							month: godate.y+"",
							year: (godate.y + 8 - 16) +"—<br>" +(godate.y+5)
						}[zoom]
					})
			}
			...
			var choose = calend.S("[choosen]").AP("span")
				.A("class", "circle");
	...
	return {
		prepare : function(input, fmt) {
			var cont = d2.S(input.parentNode).IN("div", function() { return input.nextElementSibling })
				.A("calendcont", 1)
				.A("fmt", fmt)
				/*@cc_on .A("IE8", 1)@*/
