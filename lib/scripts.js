...
Element.prototype.hasA = function(name) { return this.A(name)!=null? this : null; }
...
document.QSattr = Element.prototype.QSattr = function(a, v) { return v === undefined? this.QS('['+a+']') : this.QS('['+a+'="'+v+'"]'); }
...
Element.prototype.setDN = function(st) {
	if(st === toggle)
		...
		if(st && m.A('ref')) {
			...
			var a = this.nextElementSibling.QSA('[content-resizable]');
			for(var i = 0; i < a.length; ++i)
				X.autoResizeOnEvent(null, a[i]);
		}
	}
...
Element.prototype.showXModal = function() {
	...
	for(var e = this;  e && e.offsetParent; e = e.offsetParent) {
		...
		e.parentNode.insertBefore(cover, e);
	...
	if(!tf) {
		...
		this.insertBefore(tf, this.firstChild);
	}
	tf.safeFocus();
...
X.autoResizeOnEvent = function(e, ele) {
		var f = X.resizeFuncTable[ele.tagName];
...
var d2 = (function() {
	...
	function d2actor(elems, cont) {
		...
		self.CK = function(f) { each(self.elems, function(e) { e.CK(f) }); return self }
