  ...
  return {
    ...
    prepareCalendar : function(input, fmt) {
      ...
      var button = cont.AP("button")
        ...
        .A("onclick", "Calendar.showpopup(this)");
    },
        ...
        .draw(function() {
          ...
          this.calendar
            ...
            .N().showXModal().done(function(v) {
              ...
              d2.S(self.container).S(".calendarcover").R();
              delete calendar;
            });
  ...
  Engine.Navigator = function(cont, expmodel, ctx, editor) {
    ...
    var Node = {
      ...
      subtable : function( table, field, path ) {
        ...
        this.name = "<"+this.core.toString();
        this._text_ = table.$.caption || table.$.name;
        return this;
      },
    ...
    X.mixin({
      ...
      collectsubs : function() {
        ...
        var nodes = elems
          .filter(function(node) { return node.issubtable() && node.uniquepath() })
        this.collectnodes()
          .filter(function(node) { return node.isrelation() && !node.dictionary })
    ...
    //this.children = node.area.collectsubs()
    //  .map(function(n) { return new Region( n, root || self ) })

      this.astree = function(node) {
        this.name = 'R.'+node.name;
        this.pack = (new Pack.base(node)).region(this);
        this.children = node.area.collectsubs()
        .map(function(n) { return new Region(self.root).astree(n) })
        return this;
      }
...
var QE10 = (function(Engine) {
 /* function close() {
  ...
  function QueryEditor(ctx) {
    var self = this;
    this.container = d3.select("body>div:first-child[qe-editor]")
    ...
    this.close = function() {
      ...
      d3.select(ctx.button).style("display","inherit");
    }    
    this.draw = function() {
