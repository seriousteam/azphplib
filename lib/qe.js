var QE = (function() {
	...
	function getFilter(uri) {
		var filter_def = QE.button && QE.button.attrUpward('filter_def');
		if(filter_def && filter_def.qe) {
			return {command: filter_def.qe.cmd, args: filter_def.qe.args}
		} else {
	...
	function apply() {
		...
		if(filter_def) {
			filter_def.qe = { cmd: filter, args: args };
			applyFuncFilterT(QE.button);
			close();
		} else {
			var uri = getURI();
	...
	function drawpack() {
		...
		function update() {
			...
			node.select(".qe-nav-text")
					...
					.attr("width", function(d) { return 2*d.r+'px' } )
					.attr("height", function(d) { return 2*d.r+'px' })
				.select("body")
	...
	function start(e) {
		...
		d3.select("[qe-start]").attr("hidden", 1);
	}
	return {
		start: start,
		apply: apply,
