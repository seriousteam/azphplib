...
function checkCtrl(e) {
	...
	if(e.tagName == "A" && ckdefs.required && 
		(e.A("rid")||"").match(/^0*$/) && !e.V() ) return setCtrlError(e);
	...
	if(ckdefs.type == 'D') {
		...
		sv = d.formatDate();
		if(ckdefs.min instanceof Date) ckdefs.min = ckdefs.min.formatDate();
		if(ckdefs.max instanceof Date) ckdefs.max = ckdefs.max.formatDate();
	}
	if(ckdefs.type == 'DT') {
		var d = Date.smartParse(e.V(), true);
		if(d.y==null || d.m==null || d.d==null) return setCtrlError(e)
		...
		sv = d.formatDateTime();
		if(ckdefs.min instanceof Date) ckdefs.min = ckdefs.min.formatDateTime();
		if(ckdefs.max instanceof Date) ckdefs.max = ckdefs.max.formatDateTime();
	}
	...
	X.zones.recalc(e);
	if(e.hasA('error-vcheck') && e.A('error')) return setCtrlError(e);
	var dyn_check = e.dyn_check;
	if(e.dyn_check_min !== undefined) dyn_check = dyn_check !== false &&
			sv >= e.dyn_check_min;
	if(e.dyn_check_max !== undefined) dyn_check = dyn_check !== false &&
			sv <= e.dyn_check_max;
	if(dyn_check !== undefined) return dyn_check ? clearError(e) : setCtrlError(e);
	return true;
}
...
function getCtrlValue(e) {
	...
	if(e.hasA("save_val")) {
		...
		val = Date.smartParse(e.value, false).formatDate();
		pval = e.value;
	} else if(e.tagName == "INPUT" && e.A("vtype")=='DT') {
		val = Date.smartParse(e.value, false).formatDateTime();
		pval = e.value;
...
function takeDefVals(e) {
	...
	if(e.hasA('auto-key') && !takeRidFromCmd(e.A("cmd"))) {
		if( fv[e.A()] == undefined ) {
			fv[ '+'+e.A() ] = 'dummy';
		}
	}
...
function setRelV(e, list_item) {
		...
		if(e.dynVals) X.zones.recalc(e)
	return e;
}

function saveControlVal(e) {
	//onchange here, we can intercept this in check function
	if(!checkCtrl(e)) return X.defer(e);
	...
	var p = e;
	while(name && name.charAt(0)=='*') name = (p = p.parentNode.UA('name')).A();
	...
	if(e.hasA("onsave")) {
		//onsave: 
		// code; def() - nice return, right promice
		// def(); code - wrong return
		// def().done(function(e) { code; return e; } )
...
function patchMenuItems(ns) {
	...
	for(var j = 0; j < lis.length; ++j)
		lis[j].setA("onclick","event.stopPropagation(); this.closeModal(this); return false;");
}
...
function setWithMenu(btn, onchoose) {
	...
	function worker() {
		...
		if(m.tagName == 'MENU') {
			...
			m.refill = X.throttle(function() {
				if(flt) {
					m.evalHere(flt).done(function(dd) {
							if(dd !== undefined) {
	...
	btn.setA('loading');
	return (m.A('ref-src') ? m.reloadIt() : X.defer(m))
		.done(function() {
			return m.evalHere('@advice')
		})
		.done(function(){
			btn.setA('loading', null);
			patchMenuItems(m);
			return worker();
		});
}
...
function prepareControls(cnt, init) {
	...
	function translateAttrs(s, d, attrs, copy) {
	...
	var themed = cnt.QSA("[theme][tag], [theme][list], [theme][lobload]");
	for(var i=0;i<themed.length;++i) {
		var elem = themed[i];
		var cc = findSaveCmdElem(elem);
		var def_locker = cc && (cc =  cc.UA('locker'))? cc.A('locker') : '';
		var pair = elem.nextElementSibling;
		var container = document.createElement('DIV');
		container.setA('locked', def_locker ? '' : null );
		translateAttrs(elem,container,['theme','label','wide','inline','tip','locked','suffix'])
		translateAttrs(elem,container,['vtype','readonly'], 'copy')
		...
		if(elem.hasA('list')) {
			elem.setA('onrefresh',"var cnt = this.QSA('[list]>tbody>tr[cmd]:not([sample])').length;"
				+" this.setA('count', cnt); this.setA('max-achieved', this.hasA('max') && cnt >= +this.A() ? '' : null);")
		}
	}
	...
	for(var i = 0; i < a.length; ++i) {
		...
		if(e.tagName == 'DFN' || e.tagName == 'PRE' || e.tagName == 'LI') {
			...
			if(!e.hasA('href')) e.setA('href', 'javascript:;');
			if(e.hasA('mctl-def')) {
			...
			if(e.tagName != "BUTTON") {
				if(e.hasA('add_button')) {
					var b = document.createElement("BUTTON")
						.setA("type","button")
						.setA("onclick", "event.stopPropagation(); setWithMenu(this)")
						.setA("callmenu")
					...
					if(ns.hasA('advice')) {
						...
						ne.setA("onclick", "event.stopPropagation(); setWithMenu(this)")
					}
		...
		if(ne.tagName == 'SELECT') {
			...
			for(j =0; j< m.length; ++j) {
				...
				o.setA('rt', m[j].A('rt'));
				o.setA('disabled', m[j].A('disabled'));
				ne.options.add(o);
		...
		if(a[i].A('lobload')) {
			...
			ne.innerHTML = 
					...
					+'<form method="POST" enctype="multipart/form-data" target='+framename
						+' action=/az/server/php/filer.php>'
			...
			ne.innerHTML = 
					'<div close-box=Y display=N modal=Y quick-close>'
					+'<form method="POST" enctype="multipart/form-data" target='+framename
						+' action="'+a[i].A('lobload')+'">'
	...
	for(var i = 0; i < a.length; ++i)
	{
		...
		if(e.tagName == "INPUT" && (e.A("vtype")=="3" || e.A("vtype")=="three")) {
			...
			e.hasA('required') && ne.setA( 'required' );
			ne.setA("bool-state", {'да':'yes','1':'yes', 'нет':'no','0':'no','':'?','?':'?'}
						[ e.V() ]);
			ne.innerHTML =
				'<a yes href="" onclick="bool3state(this);return false">да</a><span>/</span><a no href="" onclick="bool3state(this);return false">нет</a>';
			e.parentNode.insertBefore( ne, e);
...
function onchooseFileForm(e) {
	...
	var f = function(la) {
		if(la!==undefined) c.QSname( 'lobarg').setV(la);
		e.UT('FORM').submit();	 
	}
	if(c.A('lobarg')) {
		var la = c.evalHere('@lobarg');
		if(la.then) la.done(f)
		else f(la);
	}
	else f();
}

function createFilerFrame(e) {
	e = e.UA('lobload');
	var dv = e.QS('IFRAME');
	if(!dv) {
...
function showFilerForm(e) {
	...
	X.defer().done(function() {
		e.QS('[modal]').QS('input[type="file"]').click();
	});
...
function applyFuncFilter(e, offset, place, direct_txt, force) {
	...
	var extParams = e.A('filter_ext_params')
	if(extParams) {
		extParams = extParams.split(/\s+/)
		for(var i = 0; i < extParams.length; ++i)
			if(mode[ extParams[i] ] !== undefined)
				uri += '&filter_params['+extParams[i]+']=' + encodeURIComponent(mode[ extParams[i] ])
	}
...
var Calendar2 = (function() {
  ...
  function calendarElement(ctx) {
    ...
    self.clickYear = function() {
      stopPropagation(d2.event);
      self.pointofview = { y: self.pointofview.y };
      X.defer().done( function() { self.draw( "year" ) } )
    }
    self.clickMonth = function() {
      stopPropagation(d2.event);
      self.pointofview = { y: self.pointofview.y, m: self.pointofview.m };
      X.defer().done( function() { self.draw( "month" ) } )
    }
    ...
    function defaultzoom() {
    	if(self.container.UA("datepicker")) return "day";
    ...
    showpopup: function(o) {
      ...
      var calendar = 
        new calendarElement({
          ...
          border : { 
            min: Date.smartParse(input.evalHere("@vmin")), 
            max: Date.smartParse(input.evalHere("@vmax")) 
          },
          input : input,
...
<div><span>Согласование</span>
	...
	modal="Y" style="left:5em" onchoose="adduser(this,def)" ref-src="/user_chooser.php.t"></dl>
	<div sample-parent>
...
X.setCtrlTip = function(e,tip) {
	var t = e.parentNode.QSattr('tip')
		t && t.setA('tip', tip);
}
