...
function modificationRequest(e, fv, params) {
	...
	var key_vals = sas_coder.DecodeValList(uri.URLParam('key_vals',""));
	if((!key_vals || !key_vals[0]) && fv && e.hasA('auto-key')) {
		if( fv[e.A()] == undefined ) {
			fv[ '+'+e.A() ] = 'dummy';
		}
	}
...
function prepareControls(cnt, init) {
	...
	var swift = cnt.QSA("[swift][tag], [swift][list]");
	for(var i=0;i<swift.length;++i) {
		var elem = swift[i];
		var pair = elem.nextElementSibling;
		var container = document.createElement('DIV');
		translateAttrs(elem,container,['swift','label','wide','tip'])
