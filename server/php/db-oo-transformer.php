...
const _SQL_FUNC_KWD =
  '/^(
  AS|NULL|CAST|IS|ANY|ALL|
  SUM|MIN|MAX|AVG|COUNT|SAME|LISTAGG|LISTAGG_D|
  ...
  ASC|DESC|COALESCE|ROWNUM|LPAD_NUM|
...
class _Cmd extends _PreCmd {
  ...
  function start_alias_tree($from, &$tree = null) {
    ...
    $tree->toplevelext = reset($tree->roots);
        ...
        if(preg_match(_SQL_GROUP_KWD, $m[0])) {
          ...
          if(!preg_match(_SQL_FUNC_KWD, $m[0])) {
            if( $closest_agg === null && !isset($grp[$m[0]]) ) {
              $parts = explode('.',$m[0]);//there is an alias - it has autogenerated
			  $node = null;
  ...
  function process_path($p, &$tree, &$externals) {
	...
	foreach($path as $key=>$name) {		
		if($name==='join') {
			...
			->getCondition($node->___node->alias, count($joinpath) ? explode('.', $rel_node->alias)[0] : $tree->toplevelext->alias);
